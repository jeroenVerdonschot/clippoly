...
// func saveMeshClipPNG(path string, originalVerts []Coord, originalFaces [][3]int, clip Polygon, resultVerts []Coord, resultFaces [][3]int) error {
// 	clipCoords := trimCoords(clip[:])

// 	buildTriangles := func(verts []Coord, faces [][3]int) [][]Coord {
// 		tris := make([][]Coord, 0, len(faces))
// 		for _, f := range faces {
// 			if f[0] < 0 || f[0] >= len(verts) || f[1] < 0 || f[1] >= len(verts) || f[2] < 0 || f[2] >= len(verts) {
// 				continue
// 			}
// 			tri := []Coord{verts[f[0]], verts[f[1]], verts[f[2]]}
// 			tris = append(tris, trimCoords(tri))
// 		}
// 		return tris
// 	}

// 	originalTris := buildTriangles(originalVerts, originalFaces)
// 	resultTris := buildTriangles(resultVerts, resultFaces)

// 	all := make([][]Coord, 0, 1+len(originalTris)+len(resultTris))
// 	all = append(all, clipCoords)
// 	all = append(all, originalTris...)
// 	all = append(all, resultTris...)
...
// 	for _, tri := range originalTris {
// 		drawLoop(img, tri, project, color.RGBA{R: 30, G: 80, B: 220, A: 255}) // blue mesh
// 	}
// 	drawLoop(img, clipCoords, project, color.RGBA{R: 255, G: 215, B: 0, A: 255}) // yellow clip
// 	for _, tri := range resultTris {
// 		drawLoop(img, tri, project, color.RGBA{R: 220, G: 40, B: 40, A: 255}) // red result
// 	}
